...
function Ganttx({ match, loading, projects }) {
  ...
  useEffect(() => {
    ...
    loop1: for (const project of projects) {
      const s_tasks = [];
      const s_t_projects = [];
      let color = "";
      ...
      for (let i in project.stages) {
        ...
        for (let j in stage.phases) {
          ...
          if (phase.feedback_date) {
            ...
            if (phase.deadline_date) {
              ...
              ddl = new Date(
                start.getTime() + 1000 * 60 * 60 * 24 * stage.days_planned
              );
            }
        ...
        s_t_projects.push({
          id: stage.id,
          title: (
            <Link to={`${match.path}/${project.id}`}>{`${parseInt(i) + 1}.${
              stage.name
            }`}</Link>
          ),
          tasks: p_tasks
    ...
    if (new_t_projects.length > 0) {
      ...
      setTimelineStart(
        toLocalDate(new Date(m_start.getTime() - 0.25 * (m_end - m_start)))
      );
      setTimelineEnd(
        toLocalDate(new Date(m_end.getTime() + 0.25 * (m_end - m_start)))
      );
    }
