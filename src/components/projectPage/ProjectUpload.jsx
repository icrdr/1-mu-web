...
import { Card, Button, Row, Col, Upload, message, Modal } from "antd";
...
function ProjectUpload({ match, phase, onSuccess }) {
  ...
  const [content, setContent] = useState(
    BraftEditor.createEditorState(phase.creator_upload)
  );
  ...
  const imgRender = [...fileArray].reverse().map(item => (
    <Col key={item.id} xs={24} md={8} className="m-b:1">
      <Card cover={<ImgCard file={item} />}>
        <CloseCircleTwoTone onClick={() => removeFile(item.id)} />
        <div className="fl:r">
