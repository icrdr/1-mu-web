...
import { Button, message, Upload, Icon, Card } from 'antd'
...
function Design({ match, project, onSuccess, history }) {
  ...
  const [content, setContent] = useState(BraftEditor.createEditorState(project.design))
    ...
    {isEdit ?
      <>
        ...
        {project.client.id === meData.id || meData.role === 'Admin' ?
          <Button size='large' onClick={() => setEdit(!isEdit)} block>修改</Button> : ""}
      </>
    }
  </>
  )
