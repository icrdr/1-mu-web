...
function Basic({ location, history, menutheme, menuItems, children }) {
  ...
  const notice = (
    <>
      <Card style={{ width: "400px" }} bodyStyle={{ padding: "0px" }}>
        <Tabs
          defaultActiveKey="1"
          tabBarStyle={{ marginBottom: "0px", padding: "0px 24px" }}
        >
                ...
                return (
                  <List.Item
                    ...
                    onClick={() => {
                      ...
                      history.push(
                        `/projects/${notice.log.project.id}?phase_id=${notice.log.phase.id}`
                      );
                      setNoticeVisible(false);
                    }}
                    className={`hover ${notice.read ? "disable" : ""}`}
                    style={{ padding: "12px 24px" }}
                  >
                    <List.Item.Meta
                      avatar={
                        <Avatarx
                          url={notice.log.operator.avatar_url}
                          name={notice.log.operator.name}
                        />
                      }
                      title={title}
                      description={moment(
                        toLocalDate(notice.log.log_date)
                      ).fromNow()}
                    />
                  </List.Item>
                );
  ...
  const menu2 = (
    <Menu>
      <Menu.Item>
        <Link to="/dashboard">
          <DashboardOutlined className="m-r:.5" />
          <span>仪表盘</span>
        </Link>
      </Menu.Item>
      <Menu.Item>
        <Link to="/me">
          <SettingOutlined className="m-r:.5" />
          <span>个人设置</span>
        </Link>
      </Menu.Item>
    </Menu>
  ...
  return (
    <Layout
      style={{ minHeight: "100vh", color: menutheme === "light" ? "" : "#ccc" }}
    >
      <Affix offsetTop={0}>
        <Header
          className="p-x:3 shadow-s"
          style={menutheme === "light" ? { background: "#fff" } : {}}
        >
          <Row>
              ...
              {meData && (
                <>
                  <Dropdown
                    ...
                    onVisibleChange={flag => setNoticeVisible(flag)}
                  >
                    <div className="btn-x">
                      <Badge count={unread}>
                        <BellOutlined
