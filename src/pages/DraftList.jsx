  ...
  return (
    <>
      {lightBox != null &&
        ...
        >{getPhase(lightBox.stages[0]).upload_files.map((file, index) => { return <ImgCard key={index} file={file} /> })}
          <div className='p:2'>
            <div className='m-b:1'>{lightBox.tags.map((tag, index) => <Tag key={index}>{tag.name}</Tag>)}</div>
            <div dangerouslySetInnerHTML={{ __html: getPhase(lightBox.stages[0]).creator_upload }} />
          </div>
        </Modal>
      }
      <BackTop />
