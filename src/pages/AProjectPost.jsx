...
import { CloseCircleTwoTone, PictureFilled } from "@ant-design/icons";
import {
  Card,
  Input,
  InputNumber,
...
function PostProjectForm({ history }) {
  ...
  const { va, errors, setValues, field, handleSubmit } = useForm(
    onSubmit,
    undefined,
    validation
  );
      ...
      <Card key={k} className="m-b:2">
        <Row className="pos:r" gutter={12}>
          <Col span={12}>
            ...
            <Input
              {...field(`stages[${k}].stage_name`, undefined, true)}
              placeholder="概括阶段工作"
            />
            {errors[`stages[${k}].stage_name`] && (
              <Alert
                message={errors[`stages[${k}].stage_name`]}
                type="error"
                showIcon
              />
            )}
          ...
          <Col span={12}>
            ...
            <InputNumber
              {...field(`stages[${k}].days_planned`, 7)}
              defaultValue={3}
              min={1}
              max={56}
            />
            {errors[`stages[${k}].days_planned`] && (
              <Alert
                message={errors[`stages[${k}].days_planned`]}
                type="error"
                showIcon
              />
            )}
    ...
    <form onSubmit={handleSubmit}>
      ...
      <Row className="m-t:2">
        <Col xs={24} md={8} className="m-b:2">
          ...
          {errors["client_id"] && (
            <Alert message={errors["client_id"]} type="error" showIcon />
          )}
        </Col>
        <Col xs={24} md={16}>
          ...
          {errors["creator_id"] && (
            <Alert message={errors["creator_id"]} type="error" showIcon />
          )}
        </Col>
      </Row>
      ...
      <div className="m-t:1">
        ...
        >
          {errors["design"] && (
            <Alert message={errors["design"]} type="error" showIcon />
          )}
        </Card>
      </div>
      <div className="m-t:2">
      ...
      <Row className="m-t:2" gutter={12}>
        <Col span={12}>
          <Button
            ...
            type="primary"
            name="post"
            onClick={e => (submit = e.target.name)}
            htmlType="submit"
          >
            提交
          </Button>
        ...
        <Col span={12}>
          <Button disabled block htmlType="submit">
            保存
          </Button>
        </Col>
      </Row>
