...
function PostProjectForm({ history }) {
  ...
  const { va, errors, field, validate, handleSubmit } = useForm(
    onSubmit,
    undefined,
    validation
  );
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="m-b:1">
        ...
        {errors["admin_id[0]"] && (
          <Alert message={errors["admin_id[0]"]} type="error" showIcon />
        )}
      </div>
      <div className="m-b:1">
        <Paragraph>*小组成员</Paragraph>
        {errors["user_id"] && (
          <Alert message={errors["user_id"]} type="error" showIcon />
        )}
        <Button className="m-b:.5" onClick={addMember}>
          添加
        </Button>
        {memberList.map((member, index) => {
          ...
          return (
            <div
              className="m-b:.5"
              key={index}
              onBlur={() => validate("user_id")}
            >
              <InputNumber
                className="m-r:.5"
                {...field(`user_id[${index}]`, 1, true)}
              />
              {index > 0 && (
                <CloseCircleTwoTone
                  style={{ fontSize: "16px" }}
              ...
              {errors[`user_id[${index}]`] && (
                <Alert
                  message={errors[`user_id[${index}]`]}
                  type="error"
                  showIcon
                />
              )}
