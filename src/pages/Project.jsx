...
export default function Project({ match, location }) {
  ...
  const operation = () => {
    switch (projectData.status) {
      case "await":
        return projectData.client.id === meData.id ||
          meData.role === "Admin" ||
          meData.role === "Editor" ? (
          <Popconfirm
            title="确定如此操作么？"
            ...
            cancelText="否"
          >
            <Button size="large" disabled={isWating} type="primary" block>
              确认开始企划
            </Button>
          </Popconfirm>
        ) : null;
      ...
      case "progress":
      case "modify":
        return projectData.creator.id === meData.id ||
          meData.role === "Admin" ||
          meData.role === "Editor" ? (
          <>
            <Button
              className={isAffixed ? "shadow" : ""}
              size="large"
              type="primary"
              disabled={showUploadPlane || projectData.pause}
              onClick={() => setUploadPlane(true)}
              block
            >
              阶段成品提交
            </Button>
        ...
        return projectData.client.id === meData.id ||
          ...
          <>
            <Button
              ...
              size="large"
              type="primary"
              disabled={showFeedbackPlane || projectData.pause}
              onClick={() => setFeedbackPlane(true)}
              block
            >
              反馈建议
            </Button>
  ...
  const stageRender = () => {
    if (progressIndex === 0) {
      return (
        <Design onSuccess={() => setUpdate(!update)} project={projectData} />
      );
      ...
      return (
        <Stage
          stageData={projectData.stages[progressIndex - 1]}
  ...
  return (
    <>
            ...
            {!isSm && (
              <>
                <div className="d:i m-r:.5" style={{ lineHeight: "25px" }}>
                  审核者：
                  <Link to={"/users/" + projectData.client.id}>
                    {projectData.client.name}
                  </Link>
                </div>
                <div className="d:i" style={{ lineHeight: "25px" }}>
                  制作者：
                  <Link to={"/users/" + projectData.creator.id}>
      ...
      <Card className="m-b:2">
        <Steps
          status={stepStatus(projectData)}
          current={stepCurrent(projectData)}
        >
          <Step
            title={
              <Button type="link" onClick={() => setProgressIndex(0)}>
                设计初稿
              </Button>
            }
          ...
          {projectData.stages.map((stage, index) => (
            <Step
              key={stage.id}
              title={
                <Button type="link" onClick={() => setProgressIndex(index + 1)}>
                  {stage.name}
                </Button>
              }
          ...
          <Step
            title={
              projectData.status === "finish" ? (
                <Button type="link" onClick={() => setProgressIndex(-1)}>
                  完成
                </Button>
              ) : (
                "完成"
      ...
      <Card className="m-b:2">{stageRender()}</Card>
          ...
          renderItem={log => {
            ...
            switch (log.log_type) {
              ...
              case "resume":
                ...
                break;
              case "stage":
                title = `${log.operator.name} 修改了企划阶段`;
                break;
              case "design":
